...
const FacultyAlerts = () => {
  ...
  if (role !== "faculty") {
    return (
      <Container sx={{ mt: 4 }}>
        <Alert severity="error">Alerts are available for faculty only.</Alert>
      </Container>
    );
  }
  ...
  if (!facultyId) {
    return (
      <Container sx={{ mt: 4 }}>
        <Alert severity="warning">Faculty ID is required to load alerts.</Alert>
      </Container>
    );
  }
        ...
        <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems="center">
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Notifications sent by admins about students who need attention.
            </Typography>
          </Box>
          <Button
            variant="outlined"
            ...
            <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems="center">
              <Box flexGrow={1}>
                ...
                <Typography variant="body2" color="text.secondary">
                  ...
                  {alert.register_number ? `(${alert.register_number})` : ""} •{" "}
                  {new Date(alert.created_at).toLocaleString()}
                </Typography>
              </Box>
              <Chip label="Admin Alert" color="warning" size="small" />
            </Stack>
          </Paper>
        ))}
      </Stack>
